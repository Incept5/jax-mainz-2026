"""
LM Studio port of the Kaggle sentiment example.

Loads the Trump-tweets dataset from Kaggle (same as analyse_sentiment_kaggle.py)
and then runs each tweet through a local LM Studio model for sentiment.

LM Studio exposes an OpenAI-compatible API on http://localhost:1234/v1.

Prerequisites:
  - LM Studio running with the local server enabled (https://lmstudio.ai)
  - A Qwen3.5 4B model loaded in LM Studio
"""
...
MODEL = "qwen3.5-4b@q4_0"  # whatever identifier LM Studio shows for the loaded model
...
def main():
    ...
    print(f"\nClassifying {len(sample)} tweets via LM Studio ({MODEL})...\n")
