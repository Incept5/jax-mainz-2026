"""
Together AI port of rag_alice_in_wonderland.py.

Same chunking + cosine-similarity RAG pipeline; the only thing that changes is
the client class — `TogetherClient` replaces `OllamaClient` and talks to
api.together.xyz (OpenAI-compatible). Compare side-by-side with the original
to see exactly what shifts when moving from Ollama to a hosted provider.
"""
...
# Pick any Together-hosted embedding model your account has access to.
# m2-bert is small/cheap; BAAI/bge-large-en-v1.5 is a stronger alternative.
...
class TogetherClient:
    ...
    def get_embedding(self, text: str, task_type: str = "document") -> np.ndarray:
        # Note: unlike EmbeddingGemma, m2-bert/BGE don't need task-specific
        # prompt prefixes — pass the raw text. `task_type` is kept in the
        # signature so the calling code stays unchanged.
        del task_type
...
class GenericRAG:
    ...
    def _load_document(self):
        ...
        for i, chunk in enumerate(chunks):
            try:
                chunk.embedding = self.client.get_embedding(chunk.text, "document")
    ...
    def _retrieve_chunks(self, query: str, top_k: int = 10) -> List[Tuple[Document, float]]:
        query_embedding = self.client.get_embedding(query, "query")
...
def main():
    print("Generic RAG Demo System (Together AI)")
